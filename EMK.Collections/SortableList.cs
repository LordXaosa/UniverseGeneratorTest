...
namespace EMK.Collections
{
    ...
    /// </summary>
    [Serializable]
    public class SortableList<T> : IList<T>, ICloneable where T : class
    {
        ...
        public SortableList(IComparer<T> Comparer) { InitProperties(Comparer, 0); }
        ...
        /// <param name="Comparer">Will be used to compare added elements for sort and search operations.</param>
        /// <param name="Capacity">Capacity of the list (<see cref="ArrayList.Capacity">ArrayList.Capacity</see>)</param>
        public SortableList(IComparer<T> Comparer, int Capacity) { InitProperties(Comparer, Capacity); }
        ...
        public T this[int Index]
        {
            ...
            set
            {
                ...
                {
                    T OBefore = Index > 0 ? _List[Index - 1] : null;
                    T OAfter = Index < Count - 1 ? _List[Index + 1] : null;
        ...
        public int IndexOf(T O)
        {
            ...
            if (_IsSorted)
            {
                ...
                while (Result > 0 && _List[Result - 1].Equals(O))
                    Result--; // We want to point at the FIRST occurence
            }
            else
                Result = _List.IndexOf(O);
            return Result;
        }
        ...
        public void CopyTo(T[] array, int arrayIndex) { _List.ToArray().CopyTo(array, arrayIndex); }
        ...
        public object Clone()
        {
            SortableList<T> Clone = new SortableList<T>(_Comparer, _List.Capacity);
            Clone._List = new List<T>((T[])_List.ToArray().Clone());
        ...
        {
            SortableList<T> SL = (SortableList<T>)O;
        ...
        {
            if (_KeepSorted) foreach (T O in C) Add(O);
        ...
        {
            if (_KeepSorted) foreach (T O in C) Insert(Index++, O);
        ...
        public void LimitNbOccurrences(T Value, int NbValuesToKeep)
        {
        ...
        private bool ObjectIsCompliant(T O)
        {
            if (_UseObjectsComparison && !(O is IComparable<T>)) throw new ArgumentException("The SortableList is set to use the IComparable interface of objects, and the object to add does not implement the IComparable interface.");
            if (!_AddDuplicates && Contains(O)) return false;
            return true;
        }

        private class Comparison<T> : IComparer<T>
        {
            public int Compare(T O1, T O2)
            {
                IComparable<T> C = (IComparable<T>)O1;
